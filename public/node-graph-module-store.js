...
const nodeGraphModuleStoreCatalog = Object.freeze({
  ...
  hypersaw: {
    category: "Oscillator",
    description: "A faithful port of soundemote's own HypersawUnit/HypersawMaster dispersion circuit (see docs/reference/Hypersaw.hpp) -- a bank of up to 64 bandlimited (PolyBLEP) sawtooths spread across the phase cycle. Parameter names match the original exactly: Distribute Phase Amp scales each voice's fixed even position across the cycle, Random Phase Amp scales a fixed per-voice random offset, Drift Amp/Frequency/Jitter scale a fixed-step random walk, and Vib Amp/Offset scale a shared vibrato oscillator that multiplicatively gates the whole static (distribute+random) dispersion -- Vib Offset at 0 fully silences it, exactly per the original's formula. Reset re-randomizes each voice's phase offset and zeroes drift, matching the original's note-trigger behavior. Center voices sum to both channels; the rest alternate Left/Right. The display shows each voice's current dispersion position as a vertical line (0..1 across the width). Native C++/WASM.",
    label: "Hypersaw",
