...
const nodeGraphModuleStoreCatalog = Object.freeze({
  ...
  hypersaw: {
    category: "Oscillator",
    description: "A faithful port of soundemote's own HypersawUnit/HypersawMaster dispersion circuit (see docs/reference/Hypersaw.hpp) -- a bank of up to 64 bandlimited (PolyBLEP) sawtooths spread across the phase cycle, with Num Oscillators itself fluid (fractional voice counts fade the 'next' voice in/out smoothly). Parameter names match the original exactly, and each is an independent, additive phase-position control (not gated by one another): Distribute Phase Amp scales each voice's fixed even position across the cycle, Random Phase Amp scales a fixed per-voice random offset, Drift Amp/Frequency/Jitter scale a fixed-step random walk, and Vib Amp/Offset add a shared vibrato oscillator's contribution. Reset re-randomizes each voice's phase offset and zeroes drift, matching the original's note-trigger behavior. Center voices sum to both channels; the rest alternate Left/Right. The display shows each voice's current dispersion position as a vertical line (0..1 across the width). Native C++/WASM.",
    label: "Hypersaw",
